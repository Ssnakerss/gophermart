...
func (ag *AccrualGetter) UpdateOrderStatus(ctx context.Context, orderToUpdate *models.Order) error {
	...
	if orderToUpdate.Status == types.PROCESSED {
...
func (ag AccrualGetter) Run(ctx context.Context, intervalSec int) error {
	for {
		...
		default:
			...
			//обаботчик результата
			go func() {
				for ord := range res {
					time.Sleep(time.Millisecond * 5) // simulate db save
					ag.UpdateOrderStatus(ctx, ord)
				}
			}()
			...
			for _, o := range pendingOrders {
				wg.Add(1)
				ord := o
				//запросы к системе расчета бонусов
				go func() {
					accrual, err := ag.accSystem.GetAccrual(ord.Number)
					if err != nil {
						wg.Done()
						return
					}
...
// 				return ctx.Err() //выход из обработчика- завершаем работу
// 			}
// 			if err != nil {
// 				break
// 			}
